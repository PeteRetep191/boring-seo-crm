...
const useLoginForm = (): LoginFormApi => {
  ...
  const handleLogin = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      localStorage.setItem(SESSION_ID_KEY, response.data.result.sessionId);
      ...
      toast.error(
        `Login failed. ${(error as any).response?.data?.error || (error as Error).message || "Please try again."}`,
      );
    } finally {
      update((draft) => {
