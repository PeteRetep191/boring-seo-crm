...
// Components
import {Accordion, AccordionItem, Button, Switch, Tooltip, Chip } from "@heroui/react";
...
const INITIAL_ITEMS = [
    { id: "1", label: "US Exclusive Offer", flags: ['US', 'CA', 'MX'], bonus: "$50", raiting: 4.5, status: 'active' },
    { id: "2", label: "North America Special", flags: ['CA', 'MX'], bonus: "$30", raiting: 4.0, status: 'inactive' },
    { id: "3", label: "UK Premium Deal", flags: ['GB'], bonus: "$40", raiting: 4.2, status: 'active' },
    { id: "4", label: "European Bundle", flags: ['FR', 'ES', 'IT'], bonus: "$20", raiting: 3.8, status: 'inactive' },
    { id: "5", label: "German Discount", flags: ['DE'], bonus: "$60", raiting: 4.9, status: 'active' },
];
        ...
        >
            {MOCK_SHOWCASES.map((showcase) => (
                <AccordionItem 
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center justify-start gap-2">
                                    <Tooltip content="Toggle showcase active state" placement="top" delay={100} closeDelay={50}>
                                        <Switch
                                            size="sm"
                                            isSelected={showcase.isActive}
                                            onChange={() => { }}
                                            // className="min-h-2 h-3"
                                            classNames={{
                                                // base: "h-3",
                                ...
                                {showcase.filters.refferers.length > 0 && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <TruncatedList
                                            ...
                                            renderItem={(referrer) => <span key={referrer}>{referrer}</span>}
                                            max={5}
                                ...
                                {showcase.filters.devices.length > 0 && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <TruncatedList
                                            ...
                                            renderItem={(device) => <span key={device}>{device}</span>}
                                            max={5}
                    ...
                    className="border border-white/10 rounded-sm shadow-none"
                    indicator={
